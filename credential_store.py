...
def _load_from_dotenv():
    ...
    return {key: env_values.get(key) for key in KEY_NAMES}


def save_credentials_from_env():
    env_values = _load_from_dotenv()
    saved = []
...
def get_credentials():
    ...
    for key_name in KEY_NAMES:
        ...
        if value is None:
            value = _load_from_dotenv().get(key_name)
            if value:
                keyring.set_password(SERVICE_NAME, key_name, value)
    ...
    if missing:
        raise RuntimeError(
            "NH_APP_KEY와 NH_APP_SECRET을 안전 저장소 또는 .env에서 확인할 수 없습니다."
        )
